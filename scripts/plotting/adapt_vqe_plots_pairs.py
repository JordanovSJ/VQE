...
if __name__ == "__main__":
    ...
    db_q_adapt = pandas.read_csv('../../results/adapt_vqe_results/vip/LiH_g_adapt_gsdpwe_27-Jul-2020.csv')

    fig, ax = plt.subplots()

    ax.plot(db_iqeb['n'], db_iqeb['error'], label='IQEB-VQE')
    ax.plot(db_g_iqeb['n'], db_g_iqeb['error'], label='grad-IQEB-VQE')
    ax.plot(db_adapt_pairs['n'], db_adapt_pairs['error'], label='pairs-ADAPT')
    ax.plot(db_adapt['n'], db_adapt['error'], label='ADAPT-VQE')
    ax.plot(db_q_adapt['n'], db_q_adapt['error'], label='qubit-ADAPT-VQE')
