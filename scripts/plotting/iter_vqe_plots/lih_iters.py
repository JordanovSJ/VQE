...
if __name__ == "__main__":
    ...
    marker = '_'
    #
    # ax.plot(db_iqeb[df_col], db_iqeb['error'], label=r'QEB-ADAPT', marker=marker, linewidth=linewidth, color='blue')
    # ax.plot(db_iqeb_1[df_col], db_iqeb_1['error'], label='IQEB, r=1', marker=marker, linewidth=linewidth, color='dodgerblue')
    ax.plot(db_iqeb_3[df_col], db_iqeb_3['error'], label=r'QEB-ADAPT', marker=marker, linewidth=linewidth, color='midnightblue')

    # ax.plot(db_q_adapt[df_col], db_q_adapt['error'], label=r'qubit-ADAPT', marker=marker, linewidth=linewidth, color='green')
    # ax.plot(db_q_adapt_1[df_col], db_q_adapt_1['error'], label='q-ADAPT, r=1', marker=marker, linewidth=linewidth, color='limegreen')
    ax.plot(db_q_adapt_3[df_col], db_q_adapt_3['error'], label=r'qubit-ADAPT', marker=marker, linewidth=linewidth, color='darkolivegreen')

    # ax.plot(db_adapt[df_col], db_adapt['error'], label=r'fermionic-ADAPT', marker=marker, linewidth=linewidth, color='red')
    # ax.plot(db_adapt_1[df_col], db_adapt_1['error'], label='ADAPT, r=1', marker=marker, linewidth=linewidth, color='orangered')
    ax.plot(db_adapt_3[df_col], db_adapt_3['error'], label=r'fermionic-ADAPT', marker=marker, linewidth=linewidth, color='darkred')
